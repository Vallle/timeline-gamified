	...
	private void setupAddButtonQuickAction() {
		...
		reflectionSpace.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				saveReflection();
				insertToReflectionSpace(reflectionTitle.getText().toString());
				finish();
			}
		});
...
//        shareReflectionIntent.putExtra(Intent.EXTRA_TEXT, reflectionText.getText().toString()); 
	...
	static String testSpaceID = "team#75";
	
	public void insertToReflectionSpace(String content){
...
//		XMPPConnection.DEBUG_ENABLED = true;
		...
		ReflectionSpaceUserPreferences loginInfo = ReflectionSpaceUserPreferences.load(getApplicationContext());
		...
		if(userName == null || password == null){
			Toast.makeText(getApplicationContext(), "Login failed, please login to the reflection space again", Toast.LENGTH_SHORT).show();
			return;
		}
		...
		SpaceHandler spaceHandler = new SpaceHandler(getApplicationContext(), connectionHandler, "offlineDB");
		spaceHandler.setMode(Mode.ONLINE);
		
		
		String userJID = userName + "@" + connectionHandler.getConfiguration().getDomain();
		registerUserOnReflectionSpace(testSpaceID, userJID, getApplicationContext());
		...
		publishElementToSpace(dataObject, reflectionSpace, dataHandler, getApplicationContext());
	 ...
	 * @param context
	 */
	public void publishElementToSpace(DataObject dataObject, Space space, DataHandler dataHandler, Context context){
		try{
	 ...
	 * @return
	 */
	public List<DataObject> getAllObjectsFromSpace(String spaceID, DataHandler dataHandler){
	 ...
	 * @param userJID
	 */
	public void registerUserOnReflectionSpace(String spaceID, String userJID, Context context){
		SmackConfiguration.setPacketReplyTimeout(10000);
		ConnectionConfigurationBuilder builder = new ConnectionConfigurationBuilder(domain, appID);
		builder.setHost(serverIP);
		builder.setPort(port);
	 ...
	 */
	public void createUserOnServer(String username, String password, String name, String email){
